...
**Fast path (no API):**
4. **Frontmatter signal:** The structured meeting note (saved in Step 7) has an `attio_deal_id` field. If present, **validate the deal exists** via MCP `get-records-by-ids`. If valid → skip detection, go directly to 7.5a. If deal is missing/archived → clear the ID from frontmatter, fall through to normal detection.
